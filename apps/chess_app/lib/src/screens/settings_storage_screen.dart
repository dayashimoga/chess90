...
class _SettingsStorageScreenState extends State<SettingsStorageScreen> {
  ...
  void _exportBackup() {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Export Offline Platform JSON Backup', style: TextStyle(color: ChessTheme.textPrimary)),
        content: SizedBox(
          width: 550,
          height: 320,
          child: Column(
            ...
            children: [
              const Text('Full reversible database export (profile, skill nodes, games, review items):', style: TextStyle(fontSize: 12, color: ChessTheme.textSecondary)),
              const SizedBox(height: 8),
              Expanded(
                child: Container(
                  padding: const EdgeInsets.all(8),
                  decoration: BoxDecoration(color: ChessTheme.background, borderRadius: BorderRadius.circular(6)),
  ...
  void _importBackupPrompt() {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: ChessTheme.surface,
        title: const Text('Import Offline JSON Backup', style: TextStyle(color: ChessTheme.textPrimary)),
        content: SizedBox(
          width: 550,
          height: 250,
          child: Column(
            children: [
              const Text('Paste JSON platform export below. Schema migrations will be applied automatically.', style: TextStyle(fontSize: 12, color: ChessTheme.textSecondary)),
              const SizedBox(height: 8),
              Expanded(
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    fillColor: ChessTheme.background,
                    filled: true,
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: _buildStatCard('Database Schema', 'Version 2 (Active)', Icons.schema, ChessTheme.primaryLight)),
                const SizedBox(width: 16),
                Expanded(child: _buildStatCard('Saved Games', '$gamesCount Games', Icons.sports_esports, ChessTheme.secondary)),
                const SizedBox(width: 16),
                Expanded(child: _buildStatCard('SRS Review Items', '$reviewCount Flashcards', Icons.repeat, ChessTheme.accentGold)),
                const SizedBox(width: 16),
                Expanded(child: _buildStatCard('Skill Graph Nodes', '$nodesCount Dimensions', Icons.hub, ChessTheme.qualityBrilliant)),
              ],
            ),
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Local-First Offline Persistence', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                  const SizedBox(height: 4),
                  const Text('All database writes are executed atomically (.tmp -> rename) with zero external server dependencies.', style: TextStyle(fontSize: 12, color: ChessTheme.textSecondary)),
                  const SizedBox(height: 16),
                  Row(
                    children: [
                      ElevatedButton.icon(
                      ...
                      OutlinedButton.icon(
                        ...
                        style: OutlinedButton.styleFrom(foregroundColor: ChessTheme.textPrimary),
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Visual System & Engine Preferences', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                  const SizedBox(height: 12),
                  ListTile(
                    contentPadding: EdgeInsets.zero,
                    leading: const Icon(Icons.palette_outlined, color: ChessTheme.accentGold),
                    title: const Text('Toggle Dark / Light Theme', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
                    subtitle: const Text('Switch between tournament olive dark theme and crisp tournament light theme.', style: TextStyle(fontSize: 11, color: ChessTheme.textMuted)),
                    trailing: ElevatedButton(
                      onPressed: widget.onToggleTheme,
                      style: ElevatedButton.styleFrom(backgroundColor: ChessTheme.surfaceLight),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text(title, style: const TextStyle(fontSize: 11, color: ChessTheme.textMuted)),
        ],
      ),
    );
  }
